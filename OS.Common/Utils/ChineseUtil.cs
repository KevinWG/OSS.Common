...
namespace OS.Common.Utils
{
    public class ChineseUtil
    {
        ...
        public static string GetFirstChars(string cnStrs)
        {
            StringBuilder ret = new StringBuilder();
            foreach (char c in cnStrs)
            {
                if (c >= 32 && c <= 126)
                {
                    ret.Append(c.ToString(CultureInfo.InvariantCulture));
                }
                else
                {
                    ret.Append(GetFirstChar(c.ToString(CultureInfo.InvariantCulture)));
                }
            }
            return ret.ToString();
        }
        ...
        /// <returns></returns>
        public static string GetFirstChar(string cnChar)
        {
            Encoding gbEncoding = Encoding.GetEncoding("gb2312");

            byte[] arrCn = gbEncoding.GetBytes(cnChar);
            if (arrCn.Length > 1)
            {
                int area = arrCn[0];
                int pos = arrCn[1];
                ...
                for (int i = 0; i < 26; i++)
                {
                    ...
                    {
                        return gbEncoding.GetString(new[] { (byte)(65 + i) });
                    }
                }
                return "*";
            }
        ...
        public static string GetPinYin(string hzStrs)
        {
            Encoding gbEncoding = Encoding.GetEncoding("gb2312");

            Regex regex = new Regex("^[\u4e00-\u9fa5]$");
            StringBuilder pystring = new StringBuilder();
            char[] nowchar = hzStrs.ToCharArray();
            ...
            for (int j = 0; j < nowchar.Length; j++)
            {
                ...
                {
                    var array = gbEncoding.GetBytes(nowchar[j].ToString(CultureInfo.InvariantCulture));
